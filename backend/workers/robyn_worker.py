...
Robyn is R-native; this worker calls it via the `robyn` Python package
if available, otherwise falls back to realistic synthetic results.
"""
import uuid
import traceback
...
@celery_app.task(bind=True, name="workers.robyn_worker.run_robyn")
def run_robyn(self, job_id: str):
    db = SessionLocal()
    try:
        ...
        result.raw_output = {"model": "robyn", "version": "3.11"}
...
def _fit_robyn(df, spend_cols, channels, season_feats, config):
    try:
        # Attempt real Robyn via Python robyn package
        import robyn  # noqa: F401
        # Full Robyn integration would go here
        raise NotImplementedError("Robyn Python API integration pending")
